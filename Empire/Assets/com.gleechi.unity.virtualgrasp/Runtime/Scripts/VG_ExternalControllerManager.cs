    ...
    public class VG_ExternalControllerManager
    {
        static public void Initialize(VG_MainScript vg)
        {
            foreach (VG_Avatar avatar in vg.m_avatars)
            {
                ...
                RegisterExternalController(avatar.m_avatarID, avatar.m_primarySensorSetup.m_profile, avatar.m_primarySensorSetup.m_origin);
                ...
                RegisterExternalController(avatar.m_avatarID, avatar.m_secondarySensorSetup.m_profile, avatar.m_primarySensorSetup.m_origin);
            }
        }

        // Register an external controller type for an avatar.
        static public void RegisterExternalController(int avatarID, VG_ControllerProfile controllerType, Transform origin = null) 
        {
            ...
            foreach (VG_HandSide side in new List<VG_HandSide>() { VG_HandSide.LEFT, VG_HandSide.RIGHT })
            {
                ...
                foreach (string controller in controllerTypes)
                {
                    ...
                    switch (controller)
                    {
                        case "OculusHand": controllers[wristID].Add(new VG_EC_OculusHand(avatarID, side, origin)); break;
                        case "OVRHand": controllers[wristID].Add(new VG_EC_OVRHand(avatarID, side, origin)); break;
                        case "UnityXR": controllers[wristID].Add(new VG_EC_UnityXRHand(avatarID, side, origin)); break;
                        case "UnityXRHands": controllers[wristID].Add(new VG_EC_UnityXRHands(avatarID, side, origin)); break;
                        case "MouseHand": controllers[wristID].Add(new VG_EC_MouseHand(avatarID, side, origin)); break;
                        case "LeapHand": controllers[wristID].Add(new VG_EC_LeapHand(avatarID, side, origin)); break;
                        case "SteamHand": controllers[wristID].Add(new VG_EC_SteamHand(avatarID, side, origin)); break;
                        //case "PicoHand": controllers[wristID].Add(new VG_EC_PicoHand(avatarID, side)); break;
                        case "UnityInteractionHand": controllers[wristID].Add(new VG_EC_UnityInteractionHand(avatarID, side, origin)); break;
                        default:
